<template>
  <div>
    ...
    <el-tabs type="border-card">
      <el-tab-pane label="不污染顶级变量">
        <p style="text-indent: 50px;">
          jQuery只建立一个名为jQuery的对象，其所有的函数方法都在这个对象之下。
          其别名$可以随时交出控制权，绝对不会污染其他变量。该特性使得jQuery可以
          和javascript的其他库共存。
        </p>
      </el-tab-pane>
      <el-tab-pane label="隐式迭代">
        <p style="text-indent: 50px;">
          123
        </p>
      </el-tab-pane>
      <el-tab-pane label="链式操作方式">
        <p style="text-indent: 50px;">
          对同一个jQuery对象的一组动作，可以直接连写而无需重复获取对象。
      ...
      <el-tab-pane label="jQuery中的DOM操作">
        <ol>
          ...
          <li>
            ...
            <p>
              replaceAll()：将被选元素替换成新的HTML元素。
            </p>
            <p>
              wrap()：使用指定的HTML元素来包裹每一个被选元素。
            </p>
            <p>
              warpInner()：将每一个匹配的元素的字内容（包括文本节点）用其他结构化的标记包裹起来。
            </p>
            <p>
            ...
            <p>
              removeClass()：移除样式，如果参数为空则移除所有class。
            </p>
            <p>
              toggleClass()：切换样式，如果类名存在则删除，如果类名不存在则添加。
            </p>
            <p>
              hasClass()：是否含有某种样式，返回布尔值。
            </p>
            <p>
              html()：读取或者设置某个元素中的HTML内容。传递一个参数为获取HTML中的内容，传递两个参数为设置HTML的内容。
            </p>
            <p>
              text()：读取或设置某个元素中的文本内容，传递一个参数为获取文本内容，传递两个参数为设置文本内容。
            </p>
          </li>
        </ol>
      </el-tab-pane>
